...
import com.liyh.agreement.MessageInfo;
...
class Server_getMessage implements Runnable {
    ...
    public void run() {
        try {
            System.out.println("客户端：" + socket.getInetAddress().getHostAddress() + " 成功连接到服务器");
            ...
            while ((len = is.read(bytes)) != -1) {
                ...
                if (message.contains("心跳")) {
                    ...
                    String result = dealHeartData(message.split("：")[1]);
                    os.write(result.getBytes());
                    os.flush();
                } else {
                    OutputStream os = socket.getOutputStream();
                    String result = dealData(message);
                    os.write(result.getBytes());
                    os.flush();
                }
            }
    ...
    public static String dealData(String message) {
        // 1.集中器登录主站注册帧报文
        // 该帧为集中器发送给主站的登录注册帧
...
//        String packet = "68 32 00 32 00 68 C9 03 44 04 00 00 02 71 00 00 01 00 88 16";
        ...
        messageInfo.setDataLength(length);
        messageInfo.setDirName(dirName);
        messageInfo.setPrmName(prmName);
        messageInfo.setFunctionName(functionName);
        messageInfo.setRegionCode(regionCode);
        messageInfo.setTerminalAddress(terminalAddress);
        messageInfo.setStationAddress(stationAddress);
        messageInfo.setApplicationName(applicationName);
        messageInfo.setTpvName(tpvName);
        messageInfo.setFiName(fiName);
        messageInfo.setConName(conName);
        messageInfo.setCommand(command);
    ...
    public static String dealHeartData(String message) {
        // 2. 集中器与主站心跳帧帧报文
        ...
        //          D3~D0=9, 功能码。链路测试
        // 03 44 04 00 00    地址域A
        // 02          应用层功能码AFN=2。链路接口测试
        // 72          0111'0010. 帧序列域SEQ。无时间标签、单帧、需要确认。
...
//        String packet = "68 32 00 32 00 68 C9 03 44 04 00 00 02 72 00 00 04 00 8C 16";
        String packet = message;
        MessageInfo messageInfo = new MessageInfo();
        ...
        // 查询02H的报文格式，链路接口检测命令上行报文格式
        // 68H - L - L - 68H - C - A - AFN=02H - SEQ - 数据单元标识（DA=0） - 数据单元 - CS - 16H
        // 72 帧序列域SEQ长度为1字节，用于描述帧之间的传输序列的变化规则，由于受报文长度限制，数据无法在一帧内传输，需要分成多帧传输（每帧都应有数据单元标识，都可以作为独立的报文处理）
        // 72转换为二进制为 01110010，对应下图的相关标志为：
        // D7    D6   D5   D4    D3-D0
        // TpV  FIR	 FIN  CON  PSEQ∕RSEQ
        // 0    1     1   1     0010
        // TpV：帧时间标签有效位，TpV=0，表示在附加信息域中无时间标签Tp；TpV=1，表示在附加信息域中带有时间标签Tp
        ...
        // 04 00    信息类DT由信息类元DT1和信息类组DT2两个字节构成。
        // DT2采用二进制编码方式表示信息类组，DT1对位表示某一信息类组的1～8种信息类型，以此共同构成信息类标识Fn（n=1～248）
        // 运算规则为    Fn = DT2 * 8 + DT1   对应位的值(8421码)就是信息类标识Fn
        // 16进制	真实数值
        //  00        0
        ...
        // 8C   帧校验和（CS）是用户数据区的8位位组的算术和，不考虑进位位。
        // 用户数据区包括控制域、地址域、链路用户数据（应用层）三部分。
        // 控制域：C9
        //
        // 地址域：03 44 04 00 00
        //
        // 链路用户数据: 02 72 00 00 01 00
        //
        // 相加之和为:18C（16进制）
        //
        // CS为 最后两位 ：8C
        ...
        // 该帧为主站给集中器的确认帧：68 32 00 32 00 68 0B 03 44 04 00 00 00 62 00 00 01 00 B9 16
        // 68 32 00 32 00 68     // 帧头
        // 0B                    // 0000'1011. 控制域C。
        //                       // （主站发送）下行方向、来自从动站、（下行方向）帧计数位无效；
        //                       // 功能码=11，当PRM=0时，代表该帧报文的帧类型为请求/响应帧，
        //                       // 服务功能为链路测试，及使用于AFN=02的应用层功能码。
        // 03 44
        // 04 00
        // 00
        // 00                   // 应用层功能码。AFN=0, 确认/否认
        // 62                   // 0110'0010, 帧序列域。无时间标签、单帧、无需确认。
        // 00 00                // 信息点。DA1和DA2全为“0”时，表示终端信息点。
        // 01 00                // 信息类。F9, 终端事件记录配置设置。
        // B9 16                // 帧尾
        StringBuilder result = new StringBuilder();
        ...
        // 帧序列域SEQ:帧序列域长度为1字节，62转换为二进制为01100010，对应下图的相关标志为：
        // D7    D6   D5   D4    D3-D0
        // TpV  FIR	 FIN  CON  PSEQ∕RSEQ
        // 0    1     1   0     0010
        // TpV：帧时间标签有效位，TpV=0，表示在附加信息域中无时间标签Tp；TpV=1，表示在附加信息域中带有时间标签Tp
    ...
    public void run() {
        try {
            while (true) {
                ...
                // scanner 默认是阻塞的，所以这个死循环必须得通畅以后才可以继续执行，所以必须输入一次后才可以继续执行
